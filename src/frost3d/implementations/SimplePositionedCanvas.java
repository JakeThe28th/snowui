...
public class SimplePositionedCanvas extends SimpleCanvas implements F3DPositionedCanvas {
	...
	@Override
	public Vector2i internalpoint(Vector2i input, int outer_width, int outer_height) {
		FloatRectangle rect = Utility.getFittedRectangle(outer_width, outer_height, width, height, halign, valign, fillmode);
		Vector2d vec = Utility.getNormalizedInternalCoordinates(rect,
				new Vector2d(input.x / (float) outer_width, input.y / (float) outer_height));
		return new Vector2i((int) (vec.x * width), (int) (vec.y * height));
	}

	@Override
	public GLMesh mesh(int outer_width, int outer_height) {
